import { MostUsedPaymentTypeChart } from '@/components/charts/PaymentUsedChart'
...
import { useGetAllPaymentTypes, useGetMostUsedPaymentTypes } from '@/http/api'

export default function Pagamento() {
  const { data } = useGetAllPaymentTypes()
  const { data: mostUsedPaymentTypesData } = useGetMostUsedPaymentTypes({
    limit: '5',
  })
  return (
    <div className="flex flex-col flex-1 justify-center items-center md:items-start p-2">
      ...
      <div className="flex lg:flex-row flex-col justify-center md:justify-between gap-8 mt-4 w-full">
        <Card className="flex p-4 w-full lg:w-1/2 h-80">
          ...
          <MostUsedPaymentTypeChart data={mostUsedPaymentTypesData ?? []} />
        </Card>
